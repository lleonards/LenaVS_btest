...
export const processVideoGenerationTask = async ({
  ...
  try {
    ...
    return {
      ...
      publicUrl: uploadedVideo.url || uploadedVideo.publicUrl,
      videoUrl: `/video/download/${encodeURIComponent(storedFileName)}?storagePath=${encodeURIComponent(storagePath)}&bucket=${encodeURIComponent(uploadedVideo.bucket || '')}&downloadName=${encodeURIComponent(downloadFileName)}`,
    };
  } catch (error) {
    await cleanupTempFiles(tempFiles);
    await removeLocalFileSilently(outputPath);
