...
class VideoTaskQueue {
  constructor() {
    ...
    // Máquina de 1 vCPU: apenas 1 worker de vídeo por vez.
    const defaultWorkers = cpuCount <= 1 ? 1 : Math.max(1, Math.min(4, cpuCount - 1));
    ...
    this.maxPendingTasks = clampInteger(process.env.VIDEO_MAX_PENDING_TASKS, 8, 2, 100);
    this.syncWaitTimeoutMs = clampInteger(process.env.VIDEO_SYNC_WAIT_TIMEOUT_MS, 120000, 30000, 1800000);
    ...
    this._lastProgressPersistAt = new Map();
  }
  ...
  async _handleWorkerProgress(taskId, message) {
    const now = Date.now();
    const lastPersist = this._lastProgressPersistAt?.get(taskId) || 0;
    const nextProgress = Math.min(99, Math.max(1, Number(message.progress) || 0));

    // Grava no disco no máximo 1x a cada 2s por tarefa: evita I/O excessivo
    // em servidor de 1 vCPU / 2GB durante a renderização.
    const shouldPersist = now - lastPersist >= 2000 || nextProgress >= 99;
    ...
    if (!this._lastProgressPersistAt) this._lastProgressPersistAt = new Map();
    this._lastProgressPersistAt.set(taskId, now);
