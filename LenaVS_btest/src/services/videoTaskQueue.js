...
import { processVideoGenerationTask } from './videoGenerationService.js';
import { getHeavyTaskGate } from './heavyTaskGate.js';
import {
  ensureTaskStore,
  createTaskRecord,
  getTaskRecord,
...
class VideoTaskQueue {
  constructor() {
    // Não use os.cpus() aqui: em alguns provedores o container enxerga os
    // CPUs do host, e não o limite real contratado pelo serviço.
    this.workerConcurrency = clampInteger(process.env.VIDEO_WORKER_CONCURRENCY, 1, 1, 1);
  ...
  async _processTask(taskId) {
    ...
    // Reserve a vaga antes de qualquer await. Isso evita que várias chamadas
    // concorrentes a _drainQueue iniciem tarefas antes de atualizar o mapa.
    this.activeWorkers.set(taskId, { label: 'video-render' });
    ...
    try {
      const result = await getHeavyTaskGate().run('video-render', () => processVideoGenerationTask({
        taskId,
        userId: task.userId,
        payload: task.payload,
        onProgress: (message) => this._handleWorkerProgress(taskId, message),
      }));

      if (!settled) {
        settled = true;
        await this._handleWorkerSuccess(taskId, result);
      }
    } catch (error) {
      if (!settled) {
        settled = true;
  ...
  _drainQueue() {
    while (
      this.activeWorkers.size + this.startingTasks.size < this.workerConcurrency
      && this.queue.length > 0
    ) {
      ...
      if (this.activeWorkers.has(nextTaskId) || this.startingTasks.has(nextTaskId)) {
        continue;
      }

      this.startingTasks.add(nextTaskId);
      this._processTask(nextTaskId).catch(async (error) => {
        ...
        this.startingTasks.delete(nextTaskId);
        this._drainQueue();
      });
    }
  }
