...
const removeFileSilently = async (filePath) => {
  ...
  try {
    ...
    if (error?.code !== 'ENOENT') {
      console.warn('[lyrics-aligner] Não foi possível remover áudio normalizado:', error.message);
    }
  }
};

const normalizeAudioForAligner = (audioPath) => new Promise((resolve, reject) => {
  const normalizedPath = path.join(
    path.dirname(audioPath),
    `${path.basename(audioPath, path.extname(audioPath))}-aligner.wav`
  );
  ...
  child.on('error', (error) => {
    const wrapped = new Error(
      `Não foi possível preparar o áudio para o Lyrics Aligner: ${error.message}`
    );
    wrapped.code = error.code === 'ENOENT' ? 'ALIGNER_UNAVAILABLE' : 'ALIGNER_AUDIO_PREP_FAILED';
  ...
  child.on('close', async (code) => {
    ...
    await removeFileSilently(normalizedPath);
    const error = new Error(
      'Não foi possível converter o áudio recebido para um formato compatível com o Lyrics Aligner.'
    );
    error.code = 'ALIGNER_AUDIO_PREP_FAILED';
    error.exitCode = code;
    error.stderr = stderr;
    reject(error);
  });
...
export const alignLyrics = async ({
  ...
  const args = [
    ...
    '--batch-size', String(Number.isFinite(ALIGNER_BATCH_SIZE) && ALIGNER_BATCH_SIZE > 0 ? ALIGNER_BATCH_SIZE : 4),
  ];
  ...
  const execution = await getHeavyTaskGate().run('lyrics-aligner', async () => {
    const normalizedAudioPath = await normalizeAudioForAligner(audioPath);
    ...
    try {
      ...
      await removeFileSilently(normalizedAudioPath);
    }
  });
