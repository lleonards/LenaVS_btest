...
const vocalStemScore = (filePath) => {
  const baseName = path.basename(filePath).toLowerCase();

  if (/^vocals\./.test(baseName)) return 100;
  if (/^vocals_/.test(baseName)) return 90;
  if (baseName.includes('vocals')) return 60;
  if (baseName.includes('vocal')) return 50;
  return 0;
};

/** Localiza o stem de VOCAL (voz isolada) gerado pelo Demucs --two-stems=vocals. */
const findBestVocalStem = async (outputRoot) => {
  const files = await listFilesRecursively(outputRoot);
  const supportedFiles = files.filter((filePath) => /\.(mp3|wav|flac|ogg|m4a)$/i.test(filePath));

  const ranked = supportedFiles
    .map((filePath) => ({ filePath, score: vocalStemScore(filePath) }))
    .filter((item) => item.score > 0)
    .sort((a, b) => b.score - a.score);
...
export const createVocalsWithDemucsFromLocalFile = async (inputPath) => {
  if (!inputPath) {
    throw new Error('Arquivo de áudio local ausente para separar o vocal.');
  }
  ...
  const detectedVocalPath = await findBestVocalStem(outputRoot);

  if (!detectedVocalPath) {
    const error = new Error('Demucs concluiu o processamento, mas não foi possível localizar o vocal separado.');
    error.code = 'DEMUCS_OUTPUT_NOT_FOUND';
    error.stdout = execution.stdout;
    error.stderr = execution.stderr;
    throw error;
  ...
  let vocalsPath = detectedVocalPath;

  if (path.extname(detectedVocalPath).toLowerCase() !== '.mp3') {
    vocalsPath = path.join(jobRoot, 'vocals.mp3');
    await transcodeToMp3(detectedVocalPath, vocalsPath);
  }

  let duration = null;

  try {
    duration = roundDuration(await getAudioDurationInSeconds(vocalsPath));
  } catch (error) {
    console.warn('[demucsService] Não foi possível medir a duração do vocal:', error.message);
  }
