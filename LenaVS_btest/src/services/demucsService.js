...
import { runExclusiveHeavyTaskTracked } from '../utils/heavyTaskLock.js';

const CONFIGURED_DEMUCS_PYTHON_BIN = String(process.env.DEMUCS_PYTHON_BIN || '').trim();
const DEMUCS_VENV_PYTHON_BIN = '/opt/lenavs-demucs/bin/python3';
...
const DEMUCS_SEGMENT = String(process.env.DEMUCS_SEGMENT || '7');
const DEMUCS_OVERLAP = String(process.env.DEMUCS_OVERLAP || '0.1');
...
const resolvePythonCandidates = () => {
  const candidates = [
    CONFIGURED_DEMUCS_PYTHON_BIN,
    DEMUCS_VENV_PYTHON_BIN,
    'python3',
  ].filter(Boolean);

  // Some hosts expose /usr/bin/python3 even though Demucs is installed in
  // the dedicated venv. Prefer the venv and keep only valid absolute paths.
...
const buildRunnerCandidates = () => {
  ...
  const only = (prefix) => candidates.filter((candidate) => (
...
export const createInstrumentalWithDemucsFromLocalFile = (inputPath) => runExclusiveHeavyTaskTracked(async () => {
  if (!inputPath) {
