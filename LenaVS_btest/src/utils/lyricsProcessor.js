 ...
 *
 * AJUSTE P/ RECURSOS MÍNIMOS: os módulos pesados (tesseract.js, pdfjs-dist,
 * @napi-rs/canvas, mammoth, word-extractor e pdf-parse) agora são importados
 * de forma PREGUIÇOSA, apenas quando um arquivo desse tipo é realmente
 * enviado. Antes, o servidor inteiro carregava essa pilha de OCR na
 * inicialização, aumentando o uso de RAM de forma constante.
 */
...
const recognizePdfImages = async (filePath) => {
  // Imports preguiçosos: só carrega OCR/canvas quando há PDF escaneado.
  const [{ getDocument }, canvasModule, { createWorker }] = await Promise.all([
...
export const validateTimeFormat = (time) => {
  ...
  const validSeconds = Math.min(parseInt(seconds, 10), 59)
    .toString()
    .padStart(2, '0');
