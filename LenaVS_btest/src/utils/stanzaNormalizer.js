...
export const normalizeProjectStanzas = (stanzas = []) => {
  ...
  return stanzas.map((stanza, index) => {
    ...
    return {
      ...
      showOnlyDuringVocal: stanza?.showOnlyDuringVocal === true,
      syncConfidence: Number.isFinite(Number(stanza?.syncConfidence)) ? Number(stanza.syncConfidence) : null,
      syncVerified: Boolean(stanza?.syncVerified),
      syncLowConfidence: Boolean(stanza?.syncLowConfidence),
      vocalPresence: Boolean(stanza?.vocalPresence),
      speechActivityDetected: Boolean(stanza?.speechActivityDetected),
      syncSource: stanza?.syncSource ? String(stanza.syncSource) : null,
      syncEngine: stanza?.syncEngine ? String(stanza.syncEngine) : null,
