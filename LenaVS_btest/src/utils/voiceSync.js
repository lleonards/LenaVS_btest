...
function isLocalSyncEnabled() {
  return envFlag(process.env.LOCAL_SYNC_ENABLED, true);
}

function isPaidOpenAiSyncAllowed() {
  return envFlag(process.env.ALLOW_PAID_OPENAI_SYNC, false);
}

function getLocalSyncEngine() {
  const normalized = String(
    process.env.LOCAL_SYNC_ENGINE
    || process.env.WHISPER_LOCAL_ENGINE
    || process.env.WHISPERX_ENABLED
    || 'auto'
  ).trim().toLowerCase();
  ...
  return ['auto', 'faster-whisper', 'whisperx'].includes(normalized)
    ? normalized
    : 'auto';
}

function canAttemptWordTranscription() {
  return isLocalSyncEnabled() || (isPaidOpenAiSyncAllowed() && Boolean(process.env.OPENAI_API_KEY));
}

/**
 * Seleciona a fonte de timestamps por palavra.
 * Prioridade padrão (gratuita):
 *  1) Motor local open-source (faster-whisper/WhisperX via Python)
 *  2) OpenAI Whisper API apenas se ALLOW_PAID_OPENAI_SYNC=1
 */
async function getWordTimestamps(localAudioPath) {
  if (isLocalSyncEnabled()) {
    try {
      return await transcribeWithFreeLocalWhisper(localAudioPath);
    } catch (error) {
      console.warn('[voiceSync] Transcrição local gratuita falhou:', error.message);
    }
  }

  if (isPaidOpenAiSyncAllowed() && process.env.OPENAI_API_KEY) {
...
/**
 * Executa a transcrição local gratuita via Python usando faster-whisper
 * (padrão) ou WhisperX (opcional), retornando timestamps por palavra.
 */
async function transcribeWithFreeLocalWhisper(localAudioPath) {
  const pythonBin = process.env.WHISPER_LOCAL_PYTHON || process.env.WHISPERX_PYTHON || 'python3';
  const scriptPath = process.env.WHISPER_LOCAL_SCRIPT
    || path.join(process.cwd(), 'scripts', 'free_transcribe.py');
  ...
  try {
    ...
    const out = await new Promise((resolve, reject) => {
      ...
      child.on('close', (code) => {
        if (code !== 0) {
          return reject(new Error(`Transcritor local falhou (code=${code}). ${stderr || ''}`.trim()));
        }
    ...
    return {
      ...
      engine: json.engine || getLocalSyncEngine() || 'faster-whisper'
    };
  } finally {
...
function toLineLevelFormat(synced, stanzas, totalDur) {
  return synced.map((seg, i) => {
    ...
    if (
      !seg
      || !Number.isFinite(Number(seg.startTime))
      || !Number.isFinite(Number(seg.endTime))
      || Number(seg.endTime) <= Number(seg.startTime)
    ) {
      return {
        start: null,
        end: null,
        ...
        verified: false,
        lowConfidence: true,
        showOnlyDuringVocal: seg?.showOnlyDuringVocal ?? true,
        vocalPresence: false,
        reason: seg?.reason || 'sem-vocal-confirmado',
      };
    }
  ...
  try {
    ...
    try {
      ...
      if (words.length > 0) {
        ...
        const matchesForTiming = opts.requireVocalTimestamps
          ? repaired.matches
          : fillNullMatches(repaired.matches, stanzas, totalDur);
        const speechRefined = refineMatchesWithSpeechActivity(
          matchesForTiming,
          stanzas,
          speechSegments,
          words,
    ...
    if (opts.requireVocalTimestamps) {
      const error = new Error(
        'Não foi possível detectar timestamps do vocal na música original.'
      );
      error.code = 'VOCAL_TIMESTAMPS_NOT_FOUND';
      throw error;
    }

    if (!canAttemptWordTranscription()) {
      console.log('[voiceSync] Nenhum motor de transcrição habilitado — usando fallback silence-detect');
    }
    ...
    // ── Fallback 4: proporcional (somente para consumidores legados) ───────────
...
export async function syncStanzasWithWhisperAnchors(audioUrl, stanzas, opts = {}) {
  if (!canAttemptWordTranscription()) {
    throw new Error('Nenhum motor de transcrição está habilitado para gerar timestamps.');
  }
...
export async function chooseBestAudioCandidateByLyrics(audioUrls, stanzas, opts = {}) {
  ...
  if (!canAttemptWordTranscription()) {
    throw new Error('Nenhum motor de transcrição está habilitado para auto-seleção.');
  }
