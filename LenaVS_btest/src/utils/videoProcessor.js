...
const resolveNonOverlappingSubtitleTimeline = (stanzas = []) => {
  ...
  return (Array.isArray(stanzas) ? stanzas : []).map((stanza) => {
    const start = Number.isFinite(Number(stanza?.syncStartTime))
      ? Number(stanza.syncStartTime)
      : parseFixedTimecode(stanza?.startTime);
    const end = Number.isFinite(Number(stanza?.syncEndTime))
      ? Number(stanza.syncEndTime)
      : parseFixedTimecode(stanza?.endTime);
