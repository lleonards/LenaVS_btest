...
const gracefulShutdown = (signal) => {
  ...
  if (serverInstance) {
    serverInstance.close(() => {
      console.log('✅ Servidor encerrado limamente.');
      process.exit(0);
    });

    // Força a saída caso conexões persistentes travem o close()
    setTimeout(() => {
      console.warn('⏱️ Timeout no encerramento gracioso. Forçando saída.');
      process.exit(0);
    }, 10000).unref();
  } else {
    process.exit(0);
...
// Evita que um erro assíncrono não tratado derrube o processo sem log
