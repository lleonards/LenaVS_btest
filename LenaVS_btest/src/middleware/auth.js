...
import { verifySupabaseToken } from '../services/jwtVerifier.js';
 ...
 */
const mapJwtPayloadToUser = (payload) => {
...
const verifyAccessToken = async (token) => {
  ...
  // 1) Verificação local: JWKS (novas signing keys ES256/RS256) com cache
  //    de kid + fallback para o segredo legado HS256. Resolve o erro
  //    "unrecognized JWT kid ... for algorithm ES256" que ocorria porque o
  //    backend tentava validar tokens assinados com chaves ES256 usando o
  //    caminho antigo (segredo compartilhado) das libs de JWT.
  try {
    const payload = await verifySupabaseToken(token);
    const mappedUser = mapJwtPayloadToUser(payload);
    if (mappedUser) {
      return { user: mappedUser, error: null };
    }
  } catch (err) {
    console.warn('Falha em verifySupabaseToken:', err?.message);
  }

  // 2) service_role (defensivo)
  try {
