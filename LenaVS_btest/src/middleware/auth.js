...
/* Cache de validação de token (120s): o polling de status do vídeo não
   precisa consultar o Supabase Auth a cada requisição — em 1 vCPU / 2GB
   isso evita derrubar o servidor durante a sincronização. */
const TOKEN_CACHE_TTL_MS = 120 * 1000;
...
const verifyAccessToken = async (token) => {
  ...
  try {
    ...
    // Mantém no máximo 500 tokens em cache para não vazar memória.
    if (tokenCache.size >= 500) {
      const oldestKey = tokenCache.keys().next().value;
      tokenCache.delete(oldestKey);
    }
    tokenCache.set(token, { at: now, user: data.user });
