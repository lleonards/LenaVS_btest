...
import { verifySupabaseToken } from '../services/supabaseJwt.js';
...
/**
 * Converte o payload do JWT (GoTrue) no formato esperado pelo middleware.
 */
const buildUserFromJwtPayload = (payload = {}) => ({
  id: payload.sub || null,
 ...
 * 1º) Verificação local contra o JWKS do projeto (funciona com as novas
 *     chaves assimétricas ES256/RS256 — projetos criados após maio/2025).
 * 2º) Fallback via cliente SERVICE_ROLE (projetos legados com JWT HS256).
 * 3º) Fallback via cliente ANON.
...
const verifyAccessToken = async (token) => {
  ...
  // 1) JWKS local (chaves assimétricas novas: sb_publishable / sb_secret)
  try {
    const { payload, error } = await verifySupabaseToken(token);

    if (!error && payload?.sub) {
      return { user: buildUserFromJwtPayload(payload), error: null };
    }

    if (error) {
      console.warn('Falha na verificação local do JWT (JWKS):', error?.message);
    }
  } catch (err) {
    console.warn('Exceção na verificação local do JWT (JWKS):', err?.message);
  }

  // 2) service_role (default) — projetos legados HS256
  try {
  ...
  // 3) anon fallback (defensivo — algumas libs antigas exigem o cliente certo)
  try {
