...
/**
 * Valida o access token emitido pelo mesmo projeto Supabase configurado no
 * backend. A validação é delegada ao Auth API do Supabase:
 *
 * - funciona tanto para JWTs HS256 antigos quanto para JWTs ES256 atuais;
 * - resolve JWKS/kid no próprio Supabase;
 * - não depende de SUPABASE_JWT_SECRET, que não deve ser usado para validar
 *   tokens ES256.
 *
 * O token nunca é validado manualmente com jsonwebtoken ou com uma chave
 * copiada do painel. O cliente público é tentado primeiro porque é a mesma
 * credencial usada pelo frontend para emitir a sessão; o cliente
 * administrativo fica como fallback para instalações legadas.
 */
const verifyAccessToken = async (token) => {
  if (!token) {
    return { user: null, error: 'Token não fornecido', code: 'AUTH_TOKEN_MISSING' };
  }
  ...
  for (const [clientName, client] of clients) {
    try {
      ...
      if (error) {
        console.warn(`Supabase rejeitou o access token (${clientName}):`, error.message);
      }
    } catch (err) {
      console.warn(`Falha ao validar o access token (${clientName}):`, err?.message);
    }
  }
...
export const authenticateToken = async (req, res, next) => {
  try {
    ...
    if (verifyError || !supabaseUser) {
      ...
      return res.status(401).json({
        code: verifyCode || 'AUTH_SESSION_INVALID',
        error: 'Sessão inválida ou expirada. Faça login novamente.'
      });
    }
