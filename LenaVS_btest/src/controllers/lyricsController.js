...
import { alignLyricsBlocks } from '../services/ctcAlignerService.js';
...
export const autoSyncLyrics = async (req, res) => {
  try {
    ...
    const result = await alignLyricsBlocks({ audioUrl, stanzas });
    ...
    console.error('[autoSyncLyrics] Erro:', error);

    const knownMessages = {
      AUDIO_REQUIRED: 'Envie a música original antes de sincronizar automaticamente.',
      LYRICS_REQUIRED: 'Envie a letra antes de sincronizar automaticamente.',
      ALIGNER_UNAVAILABLE: 'O Lyrics Aligner não está instalado no servidor. Instale as dependências Python 3.12 do backend.',
      PYTHON_VERSION_UNSUPPORTED: 'A sincronização automática exige Python 3.12 no servidor.',
      ALIGNMENT_TIMEOUT: 'A sincronização automática demorou mais do que o esperado. Tente uma música menor.',
      ALIGNMENT_INCOMPLETE: 'Não foi possível encontrar o tempo de todos os blocos. A letra permaneceu intacta.',
      ALIGNMENT_WORD_COUNT_MISMATCH: 'A letra reconhecida não correspondeu à letra enviada. A letra permaneceu intacta.',
    };
    const code = error?.code || 'ALIGNMENT_FAILED';

    return res.status(code === 'ALIGNER_UNAVAILABLE' || code === 'PYTHON_VERSION_UNSUPPORTED' ? 503 : 422).json({
      code,
      error: knownMessages[code] || error?.message || 'Não foi possível sincronizar automaticamente a letra.',
    });
  }
};
