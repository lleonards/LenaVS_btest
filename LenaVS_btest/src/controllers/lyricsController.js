...
export const synchronizeLyrics = async (req, res) => {
  ...
  try {
    ...
    const syncedStanzas = stanzas.map((stanza, index) => {
      ...
      const analysis = result?.analysis?.stanzas?.[index] || null;
      const startTime = Number(segment?.start);
      const endTime = Number(segment?.end);
      const hasConfirmedVocal = analysis?.verified === true
        && analysis?.vocalPresence === true;
      ...
      if (
        ...
        return {
          ...
          syncBlocked: true,
          syncConfidence: analysis?.confidence ?? 0,
          syncVerified: false,
          syncReason: analysis?.reason || 'sem-vocal-confirmado',
        };
      }
      ...
      return {
        ...
        syncConfidence: analysis?.confidence ?? 0,
        syncVerified: true,
        syncReason: analysis?.reason || 'vocal-confirmado',
      };
    });
    ...
    if (syncedCount === 0) {
      return res.status(422).json({
        ...
        error: 'Não foi possível detectar o vocal na música original. Tente outro arquivo ou verifique se o transcritor local gratuito (faster-whisper/WhisperX) está instalado no backend.',
      });
    }
