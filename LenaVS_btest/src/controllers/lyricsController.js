...
import { alignLyricsWithCtc } from '../services/ctcAlignerService.js';
...
export const synchronizeLyrics = async (req, res) => {
  try {
    const { audioUrl, stanzas, language = 'por' } = req.body || {};

    if (!audioUrl || typeof audioUrl !== 'string') {
      return res.status(400).json({
        code: 'ALIGNMENT_AUDIO_REQUIRED',
        error: 'Envie a referência da música original para sincronizar a letra.',
      });
    }
    ...
    if (!Array.isArray(stanzas) || stanzas.length === 0) {
      return res.status(400).json({
        ...
        error: 'Envie os blocos da letra para sincronizar.',
      });
    }

    if (stanzas.length > 500 || stanzas.some((stanza) => typeof stanza?.text !== 'string')) {
      return res.status(400).json({
        code: 'ALIGNMENT_LYRICS_INVALID',
        error: 'A estrutura dos blocos da letra é inválida.',
      });
    }

    const normalizedLanguage = String(language || 'por').trim().toLowerCase() || 'por';
    const result = await alignLyricsWithCtc({
      audioSource: audioUrl.trim(),
      stanzas,
    ...
    return res.status(error.status || 422).json({
      code: error.code || 'ALIGNMENT_FAILED',
