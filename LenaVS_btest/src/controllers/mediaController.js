...
export const createInstrumental = async (req, res) => {
  ...
  try {
    ...
    // Bucket privado -> uploadLocalFileToStorage devolve `url` já assinada.
    const instrumentalUrl = uploaded.url || uploaded.publicUrl;

    console.log('[createInstrumental] Instrumental enviado ao Supabase:', instrumentalUrl);
    ...
    const stderr = String(error?.stderr || '').toLowerCase();
    const isMemoryFailure = error?.code === 'DEMUCS_PROCESS_FAILED'
      && /(killed|out of memory|cannot allocate memory|not enough memory)/i.test(stderr);
    const status = error?.code === 'DEMUCS_UNAVAILABLE'
      ? 503
      : error?.code === 'DEMUCS_TIMEOUT'
        ? 504
        : isMemoryFailure
          ? 507
          : 500;
