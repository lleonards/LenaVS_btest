...
import { alignLyricsWithWhisperX } from '../services/whisperxService.js';
 ...
 *
 * Sincronização automática de letras com WhisperX:
 *  - O WhisperX apenas DESCOBRE os tempos de cada palavra do áudio.
 ...
 * Response: { success, language, engine: 'whisperx',
 *             stanzas: [{ id, startTime, endTime, status, matchedWords, totalWords }] }
 */
export const syncLyricsWithWhisper = async (req, res) => {
  const userId = req.user?.id || req.user?.sub || 'anonymous';
  ...
  try {
    ...
    console.log('[syncLyrics] Executando WhisperX (alinhamento de palavras)...');
    const result = await alignLyricsWithWhisperX({ audioPath, transcriptPath, language });
    ...
    const status = error?.code === 'WHISPERX_UNAVAILABLE' ? 503 : 500;
    let message = 'Não foi possível sincronizar a letra automaticamente. Tente novamente ou ajuste os tempos manualmente.';

    if (error?.code === 'WHISPERX_UNAVAILABLE') {
      message = 'O motor WhisperX não está disponível no servidor. Verifique se o requirements-whisperx.txt foi instalado.';
    } else if (error?.code === 'WHISPERX_TIMEOUT') {
      message = 'O WhisperX demorou mais do que o esperado. Tente com um áudio mais curto ou em um horário com menos carga.';
    } else if (error?.code === 'WHISPERX_PROCESS_FAILED') {
      message = 'O WhisperX não conseguiu alinhar esta música com a letra. Este recurso pode não funcionar 100% — revise os tempos manualmente.';
    }
