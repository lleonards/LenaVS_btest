...
export const syncLyricsWithAligner = async (req, res) => {
  ...
  try {
    ...
    const status = error?.code === 'ALIGNER_UNAVAILABLE' ? 503 : 500;
    ...
    } else if (error?.code === 'ALIGNER_AUDIO_PREP_FAILED') {
      message = 'O áudio enviado não pôde ser convertido para um formato compatível com a sincronização. Verifique o arquivo e tente enviá-lo novamente.';
