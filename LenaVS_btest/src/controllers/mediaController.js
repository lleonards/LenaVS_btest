...
import runHeavyExclusive from '../utils/heavyProcessGate.js';
...
export const createInstrumental = async (req, res) => {
  ...
  try {
    ...
    // Serializa processos pesados: 1 executando + 1 na fila (evita OOM em 2GB).
    const demucsResult = await runHeavyExclusive(() =>
      createInstrumentalWithDemucsFromLocalFile(sourceAudioPath)
    );
    ...
    const status = (error?.code === 'DEMUCS_UNAVAILABLE' || error?.code === 'HEAVY_BUSY') ? 503 : 500;
...
export const syncLyricsWithAligner = async (req, res) => {
  ...
  try {
    ...
    // Serializa processos pesados: 1 executando + 1 na fila.
    const result = await runHeavyExclusive(() =>
      alignLyrics({ audioPath, transcriptPath, language })
    );
    ...
    const status = (error?.code === 'ALIGNER_UNAVAILABLE' || error?.code === 'HEAVY_BUSY') ? 503 : 500;
    ...
    } else if (error?.code === 'HEAVY_BUSY') {
      message = error?.message || 'O servidor já está processando outra tarefa pesada. Tente novamente em instantes.';
