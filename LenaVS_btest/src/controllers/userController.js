...
/*
 * O avatar é guardado no bucket PRIVADO, então a URL salva é assinada e
 * expira. Aqui renovamos a assinatura a cada leitura do perfil para a imagem
 * nunca aparecer quebrada.
 */
const resolveStoredMediaUrl = async (storedUrl) => {
  const raw = String(storedUrl || '').trim();
  if (!raw) return null;

  const parsed = parseSupabaseStorageUrl(raw);
  if (!parsed || parsed.bucket === PUBLIC_STORAGE_BUCKET) return raw;

  try {
    return (await getStorageSignedUrl(parsed.storagePath, { bucket: parsed.bucket })) || raw;
  } catch (error) {
    console.warn('Não foi possível renovar a URL da mídia do perfil:', error.message);
    return raw;
  }
};

const buildUserResponse = async ({ profile = {}, metadata = {}, email = '' } = {}) => {
  ...
  const resolvedAvatarUrl = await resolveStoredMediaUrl(mergedProfile.avatar_url);
...
export const getCurrentUser = async (req, res) => {
  try {
    ...
    return res.json(await buildUserResponse({
      profile,
...
export const updateCurrentUserProfile = async (req, res) => {
  try {
    ...
    if (hasAvatarFile) {
      ...
      // Bucket privado -> usa a URL assinada (publicUrl seria null).
      avatarUrl = uploadedAvatar.url || uploadedAvatar.publicUrl;
