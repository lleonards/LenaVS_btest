...
export const generateVideo = async (req, res) => {
  try {
    ...
    // Máquina de 1 vCPU / 2GB: 4K derrubaria a renderização. Clamp para 1080p.
    const SUPPORTED_RESOLUTIONS = new Set(['360p', '480p', '720p', '1080p']);
    const SAFE_RESOLUTION = SUPPORTED_RESOLUTIONS.has(resolution)
      ? resolution
      : (['4k', '2160p'].includes(String(resolution || '').toLowerCase())
        ? '1080p'
        : '720p');
