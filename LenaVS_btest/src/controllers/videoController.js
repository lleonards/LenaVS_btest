  ...
  try {
    ...
    if (req.files) {
      ...
      for (const [fieldName, files] of Object.entries(req.files)) {
        ...
        // `url` já vem pronta para o navegador: URL pública no bucket público,
        // URL assinada no bucket privado.
        uploadedFiles[fieldName] = uploaded.url;
...
export const downloadVideo = async (req, res) => {
  try {
    ...
    // Bucket privado -> URL assinada; bucket público -> URL pública.
    const publicUrl = await resolveMediaReference(storagePath, {
      metadata: {
        storagePath,
        bucket: requestedBucket || PRIVATE_STORAGE_BUCKET,
      },
    });
