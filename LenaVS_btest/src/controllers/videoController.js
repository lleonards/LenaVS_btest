...
const getDetectedAudioUpload = (file) => new Promise((resolve, reject) => {
  ffmpeg.ffprobe(file.path, (error, metadata) => {
    ...
    const formatNames = String(metadata?.format?.format_name || '')
      .toLowerCase()
      .split(',')
      .map((value) => value.trim());

    if (formatNames.some((value) => ['mp4', 'm4a', 'mov', '3gp', '3g2', 'mj2'].includes(value))) {
      resolve({
        mimeType: 'audio/mp4',
        originalName: file.originalname.replace(/\.[^.]+$/i, '.m4a'),
      });
      return;
    }

    if (formatNames.includes('wav')) {
      resolve({
        mimeType: 'audio/wav',
        originalName: file.originalname.replace(/\.[^.]+$/i, '.wav'),
      });
      return;
    }

    if (formatNames.includes('flac')) {
      resolve({
        mimeType: 'audio/flac',
        originalName: file.originalname.replace(/\.[^.]+$/i, '.flac'),
      });
      return;
    }

    if (formatNames.some((value) => ['ogg', 'oga', 'opus'].includes(value))) {
      resolve({
        mimeType: 'audio/ogg',
        originalName: file.originalname.replace(/\.[^.]+$/i, '.ogg'),
      });
      return;
    }
  ...
  try {
    ...
    if (req.files) {
      ...
      for (const [fieldName, files] of Object.entries(req.files)) {
        ...
        const detectedUpload = getUploadedMediaKind(fieldName, file) === 'audio'
          ? await getDetectedAudioUpload(file)
          : {
        ...
        const uploaded = await uploadRequestFileToStorage(file, {
          ...
          prefix: fieldName,
          originalName: detectedUpload.originalName,
          mimeType: detectedUpload.mimeType,
        });
