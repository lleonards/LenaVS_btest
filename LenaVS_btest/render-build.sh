...
# Instala dependências Python do Lyrics Aligner (ctc-forced-aligner 1.0.2)
if [ -f requirements-aligner.txt ]; then
  echo "📦 Instalando dependências Python do Lyrics Aligner..."
  python3 -m pip install -r requirements-aligner.txt
else
  echo "⚠️ requirements-aligner.txt não encontrado. Pulando dependências do alinhador."
fi
