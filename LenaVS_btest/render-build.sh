...
# ctc-forced-aligner 1.0.2 contains a C++17 extension. The portable CPython
# build used above advertises clang++ in its sysconfig, while Render's native
# Linux build image provides GCC/G++. Force the available GNU toolchain so pip
# does not try to invoke a compiler that is not installed.
if ! command -v gcc >/dev/null 2>&1 || ! command -v g++ >/dev/null 2>&1; then
  echo "❌ O toolchain C/C++ do Render (gcc/g++) não está disponível." >&2
  exit 1
fi
export CC=gcc
export CXX=g++
export CFLAGS="${CFLAGS:-} -fPIC"
export CXXFLAGS="${CXXFLAGS:-} -std=c++17 -fPIC"
echo "🛠️ Toolchain CTC: CC=${CC}, CXX=${CXX}"
...
if [ -f requirements-demucs.txt ]; then
  ...
  CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" \
    "$PYTHON_BIN" -m pip install --no-cache-dir -r requirements-demucs.txt
