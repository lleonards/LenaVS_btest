...
if [ -f requirements-whisperx.txt ]; then
  echo "📦 Instalando dependências Python do WhisperX (sincronização de letras)..."
  python3 -m pip install --upgrade pip setuptools wheel
  python3 -m pip install -r requirements-whisperx.txt
else
  echo "⚠️ requirements-whisperx.txt não encontrado. Pulando dependências do WhisperX."
fi
