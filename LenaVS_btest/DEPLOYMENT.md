...
- `SUPABASE_PUBLISHABLE_KEY` e `SUPABASE_SECRET_KEY`: nomes alternativos aceitos
  pelas chaves novas do Supabase. Use-os no lugar das variáveis acima quando o
  seu painel não exibir as chaves legadas.
- `SUPABASE_JWT_SECRET`: não configurar. O backend valida o access token pelo
  Auth API do Supabase, compatível com HS256 e ES256/JWKS; não faz validação
  manual com JWT secret.
