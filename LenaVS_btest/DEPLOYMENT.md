...
- Se não usar Docker: Node.js 22.x, FFmpeg instalado no servidor e Python 3.12 + PyTorch CPU + Demucs + ctc-forced-aligner 1.0.2 instalados no servidor
...
4. O container já instala Node 22, Python 3.12, FFmpeg, PyTorch CPU, Demucs e ctc-forced-aligner 1.0.2.
...
- A sincronização automática usa `POST /api/lyrics/auto-sync` com ctc-forced-aligner 1.0.2. Ela recebe os blocos já criados pelo editor e só devolve os tempos; não resegmenta a letra.
- Se o serviço for criado como Render nativo em vez de Docker, use `bash render-build.sh` como Build Command. Esse script baixa o CPython 3.12 para `.runtime/python312` quando necessário e valida o Lyrics Aligner antes de finalizar.
- O `ctc-forced-aligner==1.0.2` possui uma extensão C++17. O build força `CC=gcc` e `CXX=g++`, evitando que o CPython portátil tente chamar um `clang++` inexistente no ambiente Linux do Render.
