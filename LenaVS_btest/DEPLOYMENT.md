...
- Se não usar Docker: Node.js 22.x, FFmpeg instalado no servidor e Python 3.12 + PyTorch CPU + Demucs + `ctc_forced_aligner==1.0.2` instalados no servidor
...
4. O container já instala Node 22, Python 3.12, FFmpeg, PyTorch CPU, Demucs e `ctc_forced_aligner==1.0.2` de forma compatível.
...
- A sincronização automática usa `POST /api/lyrics/align` e executa o `ctc-forced-aligner` 1.0.2 no próprio container Linux.
- O modelo ONNX do aligner é baixado no primeiro uso e reutilizado em `/tmp/lenavs/ctc_forced_aligner/model.onnx`.
