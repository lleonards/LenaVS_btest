...
Quando o modelo consegue carregar, o alinhador devolve o tempo de cada palavra;
a agregação por bloco acontece em `scripts/lyrics_align.py`. Em um servidor de
1 CPU / 2 GB, o modo padrão `auto` usa uma distribuição proporcional segura
quando o pacote/modelo não está instalado, é encerrado por falta de memória ou
estoura o timeout. Assim o editor continua funcionando, marcando os tempos
estimados em vez de perder a operação inteira.
...
| `LYRICS_ALIGN_MODE` | `auto` | `auto` = real com fallback proporcional; `align` = exige IA; `proportional` = econômico |
...
| `LYRICS_ALIGN_WINDOW_SECONDS` | `15` | Tamanho da janela de áudio, menor para reduzir RAM |
| `LYRICS_ALIGN_CONTEXT_SECONDS` | `1` | Contexto/sobreposição entre janelas |
...
- No modo `auto`, se o servidor ficar sem memória/tempo, a resposta usa
  distribuição proporcional e marca os blocos como estimados. No modo `align`,
  o frontend recebe uma mensagem clara (HTTP 507/504) e a letra original
  permanece intacta.
