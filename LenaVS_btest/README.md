...
## 🎤 Sincronização Automática de Letras (Lyrics Aligner)

Recurso que analisa a música junto com a letra fornecida pelo usuário e
preenche automaticamente os tempos (mm:ss) de cada bloco de letra **já
existente** no editor.

- O alinhamento usa o pacote **`ctc-forced-aligner==1.0.2`** (Python 3.12).
  **Não usar a versão 2.x** — a API é diferente.
- O Lyrics Aligner descobre o tempo (start/end) de cada palavra; a LenaVS usa
  esses tempos apenas para definir início/fim dos blocos.
- **A estrutura da letra é preservada exatamente como o usuário criou**: o
  alinhador não cria, exclui, divide, junta ou reorganiza blocos. O mapeamento
  é feito por índice sequencial de palavras (blocos → palavras na ordem do
  usuário → alinhamento → tempos de volta por índice).
- **Atomicidade**: a resposta de sucesso só sai quando TODOS os blocos foram
  ...
  alterado — blocos e letra permanecem intactos.
- O editor continua mostrando apenas minutos e segundos (sem milissegundos).

### Instalação (servidor)

```bash
# Python 3.12. O pacote compila uma extensão C++ - é preciso ter um
# compilador (g++ / build-essential) e ffmpeg no servidor.
python3 -m pip install -r requirements-aligner.txt
```

O `render-build.sh` já instala `requirements-aligner.txt` automaticamente.

No primeiro uso, o modelo MMS_FA (multilíngue, cobre português, licença MIT)
é baixado e armazenado em cache pelo torchaudio (~1 GB).

### API

`POST /api/lyrics/sync` (autenticado) cria uma tarefa e responde imediatamente
com `202`. Isso evita o erro de rede causado por manter uma requisição aberta
durante o download do modelo e o alinhamento, que podem levar vários minutos.

```jsonc
// Request
{ "audioUrl": "https://.../musica-original.mp3", "blocks": [{ "text": "Eu quero cantar" }, ...] }

// Success 202
{ "success": true, "async": true, "taskId": "...", "task": { "status": "queued" } }

// Acompanhamento (autenticado)
GET /api/lyrics/sync/:taskId

// Em processamento
{ "id": "...", "status": "running", "progress": 25, "message": "..." }

// Concluído
{ "id": "...", "status": "completed", "progress": 100,
  "timings": [{ "start": "00:10", "end": "00:12" }, ...], "words": 6 }

// Falha — nenhum tempo é alterado
{ "status": "failed", "code": "LYRICS_SYNC_FAILED",
  "error": "Não foi possível sincronizar a letra..." }
```
...
|---|---|---|
| `LYRICS_ALIGNER_PYTHON_BIN` | `python3` | Binário do Python 3.12 |
| `LYRICS_ALIGNER_TIMEOUT_MS` | `600000` | Timeout da análise |
| `LYRICS_ALIGNER_MODEL_TYPE` | `MMS_FA` | Modelo torchaudio (ex.: `VOXPOPULI_ASR_BASE_10K_ES`) |
| `MEDIA_DOWNLOAD_TIMEOUT_MS` | `600000` | Tempo máximo para baixar o áudio antes do alinhamento |
| `LYRICS_SYNC_TASK_TTL_MS` | `3600000` | Tempo de retenção das tarefas em memória |

Para o CORS, configure `FRONTEND_URL` ou `ALLOWED_ORIGINS` com a URL exata
do frontend. O backend também aceita subdomínios HTTPS do Render e da
`lenavs.com`.

### Frontend

No painel **Arquivos**, abaixo do botão "Criar Instrumental com IA", há o botão
**"sincronizar automaticamente"** com a observação "Este recurso pode não ser
100% funcional". Ele fica disponível apenas quando o usuário fez upload da
música original **e** da letra.
