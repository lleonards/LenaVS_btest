...
## Sincronização automática da letra (novo)

Endpoint: `POST /api/lyrics/auto-sync` (autenticado).

O sistema detecta internamente o momento em que **cada palavra** é cantada no
áudio original e usa esses tempos para definir o **início** e o **fim** de cada
**bloco** da letra — exatamente na organização criada na LenaVS, sem dividir,
juntar ou reorganizar blocos. Os tempos retornados são sempre `MM:SS`
...
```json
{
  ...
  "stanzas": [{ "id": "...", "text": "Eu quero cantar" }]
}
```
...
```json
{
  "success": true,
  "engine": "groq",
  "stanzas": [{ "id": "...", "startTime": "00:10", "endTime": "00:12", "matched": true }],
  "stats": { "blocks": 2, "syncedBlocks": 2, "estimatedBlocks": 0, "wordsDetected": 312 }
}
```

### Configuração (API 100% gratuita)

Motor principal: **Groq Whisper API** (gratuita, com timestamps por palavra).

| Variável   | Valor                                                        |
| ---------- | ------------------------------------------------------------ |
| `GROQ_API_KEY` | chave gratuita criada em https://console.groq.com/keys (começa com `gsk_...`) |

Opcionais: `GROQ_WHISPER_MODEL` (padrão `whisper-large-v3-turbo`),
`GROQ_WHISPER_LANGUAGE` (ex.: `pt`; vazio = automático).

Fallback local sem chave: instale `pip install whisperx` e defina
`WHISPERX_ENABLED=1` (usa `scripts/whisperx_transcribe.py`).

O `ffmpeg` (já presente no Dockerfile) é usado para comprimir o áudio antes
do envio à API, mantendo o arquivo abaixo do limite gratuito de 25 MB.
