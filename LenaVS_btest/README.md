...
## Sincronização automática gratuita

O backend foi ajustado para priorizar um pipeline 100% local/gratuito para sincronização:

- `ffmpeg` para preparar a guia vocal e detectar trechos com/sem voz
- `demucs` para melhorar o isolamento do vocal
- `faster-whisper` como transcritor principal com timestamps por palavra
- `WhisperX` opcional, caso você queira forçar esse motor via variável de ambiente

Variáveis recomendadas:

- `LOCAL_SYNC_ENABLED=1`
- `LOCAL_SYNC_ENGINE=auto` (`auto`, `faster-whisper` ou `whisperx`)
- `ALLOW_PAID_OPENAI_SYNC=0` para manter a sincronização estritamente gratuita

Com esse fluxo, as estrofes sincronizadas só permanecem visíveis quando há evidência real de vocal. Projetos salvos também preservam os sinais `syncBlocked`, `syncVocalPresence` e `vocalPresence`, evitando que legendas reapareçam em trechos instrumentais após reabrir o projeto.
