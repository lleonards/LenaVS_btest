...
A rota `POST /api/media/instrumental` executa o Demucs localmente no servidor, sem depender de Replit ou Replicate. O frontend envia a URL da música original, o backend baixa o arquivo, procura primeiro o Python dedicado `/opt/lenavs-demucs/bin/python3` (e só depois o Python do sistema), executa uma separação limitada a 1 CPU com segmentos curtos, faz upload do `no_vocals` para o Supabase e devolve a URL final. O fluxo é independente da sincronização de letras.

Em uma máquina de 1 CPU / 2 GB, Demucs e o alinhador compartilham somente um lock de recurso para não serem executados ao mesmo tempo; um não é usado para fazer o trabalho do outro.
