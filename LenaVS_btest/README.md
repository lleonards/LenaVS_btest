...
- sincroniza automaticamente os blocos existentes com ctc-forced-aligner 1.0.2
...
- ctc-forced-aligner 1.0.2 (Python 3.12, timestamps por palavra)
- Multer
- Jimp
...
- Se rodar sem Docker: Node.js 22, FFmpeg instalado e Python 3.12 + PyTorch CPU + Demucs + ctc-forced-aligner 1.0.2 instalados
...
## Sincronização automática da letra

A rota `POST /api/lyrics/auto-sync` baixa a música original, envia os textos dos blocos atuais ao
`ctc-forced-aligner==1.0.2` usando Python 3.12 e agrupa os timestamps de palavra de volta nos
mesmos blocos e na mesma ordem. O frontend exibe somente minutos e segundos. Se o alinhamento
falhar ou a quantidade de palavras não conferir, a resposta é um erro e nenhum bloco é alterado.

### Python 3.12 no Render

O `render-build.sh` não depende de um Python pré-instalado na imagem nativa do Render.
Quando `python3.12` não existe, `scripts/ensure_python312.sh` baixa automaticamente uma
distribuição portátil do CPython 3.12 para `.runtime/python312`, instala todas as dependências
nessa mesma instalação e valida o import do `ctc-forced-aligner` antes de concluir o build.
O backend também procura essa cópia no runtime, portanto o processo iniciado pelo Render usa o
mesmo Python 3.12 que recebeu as dependências.

No modo Docker, o `Dockerfile` usa `python:3.12-slim-bookworm` e executa a mesma validação
...
- Python 3.12 / pip (automaticamente, mesmo quando o Render não fornece `python3.12`)
- toolchain GNU `gcc`/`g++` com C++17 para compilar a extensão nativa do alinhador
- PyTorch CPU
- demucs
