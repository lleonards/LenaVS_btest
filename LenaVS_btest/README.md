...
- sincroniza automaticamente a letra existente com timestamps de palavras usando WhisperX
...
A rota `POST /api/lyrics/sync` recebe a URL pública da música original e os blocos já criados pelo editor. O WhisperX retorna os timestamps por palavra. O backend apenas associa essas palavras aos blocos recebidos, mantendo a quantidade, a ordem e o texto de cada bloco. Os campos `startTime` e `endTime` enviados ao editor são formatados em `MM:SS`; os timestamps precisos por palavra ficam em `wordTimings`.

O Dockerfile já instala Python 3.11 do Debian Bookworm, WhisperX, PyTorch CPU e FFmpeg, além de configurar os padrões de execução. Assim, o serviço Docker do Render não precisa de uma variável adicional ou de uma alteração manual no painel.
